...
public class ViewtypeEditor extends MultiPageEditorPart implements IResourceChangeListener{
	...
	public void createViewtypeTreeEditorPage()
	{
		...
		org.eclipse.emf.common.util.URI emfURI = org.eclipse.emf.common.util.URI.createURI(fileEditorInput.getPath().toString());
		...
		try
		{
			...
			CheckboxTreeViewer treeViewer = new CheckboxTreeViewer(body, SWT.BORDER);
			...
			treeViewer.setContentProvider(new fr.inria.atlanmod.emfviews.ui.wizard.view.AttributeSelectionContentProvider());
			
			treeViewer.setLabelProvider(new AdapterFactoryLabelProvider(new ReflectiveItemProviderAdapterFactory()));
			
			treeViewer.setInput(((Viewtype)viewtypeResource).getContributingEpackages());
	...
	void createViewtypePropertiesEditorPage() 
	{
		...
		Section contributingMetamodelsSection=toolkit.createSection(viewtypeForm.getForm().getBody(), Section.DESCRIPTION | Section.TWISTIE | Section.TITLE_BAR);
		contributingMetamodelsSection.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true, false, 1, 1));
		...
		Button btnAddMetamodel=toolkit.createButton(composite_1, "Add", SWT.NONE);
		...
		btnAddMetamodel.addSelectionListener(new SelectionAdapter(){
			...
			public void widgetSelected(SelectionEvent evt) {
				AbstractSelection dialog=new ModelSelection(container.getShell(), Messages.getString("VirtualModelFileScreen.InputMetamodelCreation"), inputMetaModelPaths,ModelSelection.INPUTMETAMODEL);
				dialog.create();
				if (dialog.open() == Dialog.OK) {
					...
					case ModelSelection.INPUTMETAMODEL:
						...
						IDocument theDoc=viewtypeTextEditor.getDocumentProvider().getDocument(viewtypeTextEditor.getEditorInput());
						...
						if(metamodels.length()>0)
							metamodels=metamodels+","+result[1].toString();
						else
							metamodels=result[1].toString();
						String updatedVMM=updateViewtype("contributingMetamodels="+metamodels, "correspondenceModel="+correspondenceModel, "correspondenceModelBase="+correspondenceModelBase);
		...
		btnRemoveMetamodel.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent evt) {
				...
				String updatedVMM=updateViewtype("contributingMetamodels="+mm, "correspondenceModel="+correspondenceModel, "correspondenceModelBase="+correspondenceModelBase);
				theDoc.set(updatedVMM);
			}
		});		
		
		Section sctnLinksTechnology = toolkit.createSection(viewtypeForm.getForm().getBody(), Section.DESCRIPTION | Section.TWISTIE | Section.TITLE_BAR);
		...
		modifyLinksDslButton.addSelectionListener(
				new SelectionAdapter(){
					@Override
					public void widgetSelected(SelectionEvent evt) {
						ResourceListSelectionDialog selectLinksDslDialog = new ResourceListSelectionDialog(container.getShell(), ResourcesPlugin.getWorkspace().getRoot(), IResource.FILE); 
						...
						if (selectLinksDslDialog.open() == Dialog.OK)
						{
							...
							String updatedVMM=updateViewtype("contributingMetamodels="+metamodels, "correspondenceModel="+correspondenceModel, "correspondenceModelBase="+correspondenceModelBasePath);
							theDoc.set(updatedVMM);
						}
					}
	...
	private String updateViewtype(String contributingMetamodelsProperty, String correspondenceModelProperty,String correspondenceModelBaseProperty)
	{
	...
	public void doSave(IProgressMonitor monitor)
	{
		...
		if(structuralFeaturesToHide!=null && structuralFeaturesToHide.size()>0)
		{
			for (Object attributeToHide : structuralFeaturesToHide)
			{
				...
				if(eo instanceof EStructuralFeature)
				{
					...
					if(!completeContainingClasses.contains(owningClass))
					{
						...
						if(!completeContainingEpackages.contains(containingPack))
						{
					...
					else
					{
						...
						for (int i = 0; i < containingClasses.size() && !found; i++)
						{
							...
							String tempId=tempC.getEPackage().getNsURI().concat(tempC.getName());
							if(tempId.compareToIgnoreCase(owningClass.getEPackage().getNsURI().concat(owningClass.getName()))==0)
							{
								found=true;
								tempEClass=tempC;
							}
		...
		//Guardar el metamodelo de los atributos tambien.
	}
	/**
	...
	protected void pageChange(int newPageIndex) {
		...
		if(newPageIndex == 1)
		{
			...
			String editorText =
					viewtypeTextEditor.getDocumentProvider().getDocument(viewtypeTextEditor.getEditorInput()).get();
			Properties properties= createPropertiesFromViewtypeInTextEditor(editorText);
			String metamodels = properties.getProperty("contributingMetamodels");
			...
			Display.getDefault().asyncExec(new Runnable(){
				public void run(){
					...
					for (int i = 0; i<pages.length; i++){
						if(((FileEditorInput)viewtypeTextEditor.getEditorInput()).getFile().getProject().equals(event.getResource())){
