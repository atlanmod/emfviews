				...
				if (nameFeature != null) {
					...
					if (pathComp.equals(objName)) {
						...
						if (possibleMatchedObject instanceof EClass) {
							// since eContents does not get attributes and references from EClasses, this
							// extra step is necessary to get it
							EClass possibleMatchedClass = (EClass) possibleMatchedObject;
							Iterator<EStructuralFeature> featIterator = possibleMatchedClass.getEAllStructuralFeatures()
									.iterator();
							while (featIterator.hasNext()) {
								EStructuralFeature possibleFeature = featIterator.next();
								if (possibleFeature instanceof EAttribute || possibleFeature instanceof EReference) {
									objs.add(possibleFeature);
								}
							}
						}
