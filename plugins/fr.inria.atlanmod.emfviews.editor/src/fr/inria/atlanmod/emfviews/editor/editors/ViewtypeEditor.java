...
public class ViewtypeEditor extends MultiPageEditorPart implements
	...
	public void createViewtypeTreeEditorPage() {
		...
		try {
			...
			ArrayList<EObject> hiddenElemens = viewtype.getHiddenElements();
	...
	private String updateViewtypeDefinition(String existingViewtypeDefinition, String addedMetamodelUri, String dslBase)
	{
	...
	public void doSave(IProgressMonitor monitor) {
		...
		if (elementsToFilter != null
				&& elementsToFilter.size() > 0)
		{
			String linksResourcePath= createPropertiesFromViewtypeInTextEditor(viewtypeTextEditor.getDocumentProvider()
					.getDocument(viewtypeTextEditor.getEditorInput()).get()).getProperty("correspondenceModel");
			...
			Resource modelResource = new ResourceSetImpl().getResource(org.eclipse.emf.common.util.URI.createURI(linksUri.toString()), true);
			...
			for (Object elementToFilter : elementsToFilter) {
				if (elementToFilter instanceof EStructuralFeature)
				{
					...
					Filter filter = EmfViewsUtil.createFilter("filter"+estFeature.getEContainingClass().getName(), "", true);
					LinkedElement filterLinkedElement = EmfViewsUtil.createLinkedElement(estFeature.getEContainingClass().getName(), estFeature.getEContainingClass().getEPackage().getNsURI(), "//"+estFeature.getEContainingClass().getName(),estFeature.getName());
				...
				else if(elementToFilter instanceof EClass)
				{
						...
						Filter filter = EmfViewsUtil.createFilter("filter"+tempEclass.getName(), tempEclass.getName()+".allInstances()",false);
						LinkedElement filterLinkedElement = EmfViewsUtil.createLinkedElement(tempEclass.getName(), tempEclass.getEPackage().getNsURI(), "//"+tempEclass.getName(),null);
