...
* Concepts
** Views
Views are lightweight objects that aggregate several contributing models, and
allows you to access these models transparently.

There are three ways to create views:

1. by writing an [[*~eview~ and ~eviewpoint~ files][~eview~ (and an ~eviewpoint~) file]];
...
through the ~org.eclipse.emf.ecore.extension_parser~ extension point, which
means you are able to open ~eview~ and ~eviewpoint~ files in model editors such
...
explicit scheme, the file one is the default.  Relative file paths are resolved
...
- ~from~ :: Specifies the metamodels contributing to the viewpoint.
     ...
     The metamodel are given through their namespace URIs, and must be given an
     alias which you will use in the ~select~ clause.
...
[[https://raw.githubusercontent.com/atlanmod/emfviews/master/dsls/vpdl/org.atlanmod.emfviews.vpdl/src/org/atlanmod/emfviews/vpdl/Vpdl.xtext][The full Xtext grammar]] can be found in the source code, but the following is an
abridged version:
...
You can browse the contents of the viewpoint using ~Viewpoint.getRootPackage~,
and the contents of the view using ~View.getVirtualContents~.
...
viewpoints and views, the following description applies to both levels, even
...
The ~whitelist~ flag changes the meaning of the filtes.  If the ~whitelist~
...
Thus, you can create virtual associations between one class of a contributing
...
determines whether its cardinality.  The ~type~ attribute describes the
...
Its main purpose is to populate virtual associations based on the contents of
the contributing model.

By default, EMF Views can use ECL files to create weaving models.  Such files
...
The third step is creating the view by passing the viewpoint and a list of
...
Lastly, we navigate the view to print the name of the components inside it.
...
the same reasons, testing for instances with ~instanceof~ will not work:
