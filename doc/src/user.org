...
3. by using the EMF Views [[*API][API]].
...
That is, each line is contains a single ~KEY=VALUE~ pair, where both ~KEY~ and
~VALUE~ are strings:
...
For ~eviewpoint~ files, there are only two valid keys:
...
above schemes, or if the URI begin with ~http://~ then that is taken to be the
...
- ~select~ :: Specifies which features from the contributing metamodels to
     include in the view, and which new associations (if any) to create between
     models.

     At the very least, you must include one feature must (otherwise the view
     will be empty and you could use an empty weaving model directly instead of
     writing a VPDL file).
     ...
     New associations are created between two classes with a ~join~ statement:
...
- ~from~ :: Specifies the metamodels contributing to the viewpoint.
     ...
     The metamodel are given through their namespace URIs, and are then given an
     alias which you use in the ~select~ clause.
...
# TODO: describe each weaving model directive (filters, new associations, new
# elements)
...
By default, EMF Views can use ECL files to create weaving models.  Such
typically contain one rule for each virtual association.  Here is an example ECL
file containing two rules:
...
extension (e.g., "ecl") and a class implementing the ~IVrtualLinksDelegate~
