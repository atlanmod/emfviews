...
query and transform them as you would do with any regular model.  This comes in handy
when you want to deal with several related models, for instance.
...
Views can also augment the contributing models with new elements, attributes or associations.  
This is useful for instance when combining related models: you can add
...
files, or can be created programmatically.  Moreover, EMF Views currently comes with two
DSLs that ease the definition of views: VPDL and MEL.
#TODO - Put pointers to the VPDL and MEL sections

** Getting Started
[[*Creating a view manually][Creating a view manually]] - This section will take you through the main features of
EMF Views.

* Concepts
** Views
In EMF Views, views are actually lightweight (virtual) models that can rely on one or several contributing models.
Thus, they allow you to access to these models in transparently.

There are three ways to create a same view:
...
approach is probably more flexible.  Using the API should be preferred when creating
...
through the ~org.eclipse.emf.ecore.extension_parser~ extension point. This
means you are able to natively open ~eview~ and ~eviewpoint~ files in standard EMF model editors such
as the Sample Ecore Model Editor, or the [[eclipse:/topic/org.eclipse.modisco.infrastructure.doc/mediawiki/model_browser/user.html][MoDisco Model Browser]].
...
explicit scheme, the file one is used by default.  Note that relative file paths are resolved
relatively to the location of the ~eviewpoint~ file.
...
The syntax of VPDL is freely inspired by the syntax of the SQL's ~SELECT~ statement.
...
- ~select~ :: Specifies which contributing metamodel features to include in the
     ...
     be populated based on model contents.  For that, you have to use the ~where~
     clause.
...
viewpoints and views (respectively), the following description applies to both levels, even
though we only use the terms "models" and "views".  For elements that are
handled differently by viewpoints and views, we make the distinction explicit.
...
Its ~name~ attribute is used as part of the viewpoint's namespace URI.
Views do not make use of the ~name~ attribute.
...
~whitelist~ flag is true, then the view will include no element, unless
they are explicitly filtered in.
...
Thus, you can possibly create a virtual association between one class of a contributing
...
the virtual association is a containment one.  A virtual association can have one
...
determines its cardinality (1 or 0..1).  The ~type~ attribute describes the
primitive type of the property.  The following types are supported:
...
In the current implementation, EMF Views can use ECL files to create weaving models.  Such files
typically contain one rule for each virtual association.  Here is one ECL file
with two rules:
...
To define the view, we must first define a viewpoint, which acts as a (virtual) metamodel for
...
Here, if we have a book and a publication that refer to that same book, then
we want to add all chapters of this book to the association.  Thus, the predicate
...
created two views combining two models using this viewpoint.  Note that
while this method of creating views with ~eviewpoint~ and ~eview~ files is
suitable for creating small-scale views interactively (e.g. for development purposes), EMF Views offers two
other (more user-friendly) methods to create views: programmatically and using VPDL.
...
*** Writing a VPDL file
VPDL, standing for /ViewPoint Description Language/, is a domain-specific
language for easing the specification of viewpoints and the creation of corresponding views using EMF Views.
...
one for a view (yet).  You just have to point to the generated files, and specify
...
the same reasons, testing for instances with ~instanceof~ will not work with the current version of EMF Views:
