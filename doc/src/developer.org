...
# TODO: fuzzy links to other files use the built-in Org ids instead of the
# custom slugs
...
- The *plugins* folder contains the core plugins, namely ~org.atlanmod.emfviews~
  ...
  plugin (see [[file:user.org::#adding-new-matching-models][Adding new matching models]]) is used to create views from an ECL
