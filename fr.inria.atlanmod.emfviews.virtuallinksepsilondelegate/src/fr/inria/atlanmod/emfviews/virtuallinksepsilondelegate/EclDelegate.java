...
public class EclDelegate implements IVirtualLinksDelegate
{
	...
	@Override
	public void createVirtualMetamodelLinks(String linksDslFilePath, URI modelLinksURI) throws Exception  
	{
		...
		java.net.URI uri = workspace.getRoot().findMember("/"+linksDslFilePath).getLocationURI();
		...
		while ((sCurrentLine=br.readLine()) != null)
		{
			...
			{
				String metamodelAlias=sCurrentLine.substring(sCurrentLine.indexOf("_")+1, sCurrentLine.indexOf("="));
	...
	@Override
	public void createVirtualModelLinks(String linksDslFile, URI linksModel,List<Resource> inputModelsResourcesList)
			throws Exception {
		
		
		IWorkspace workspace = ResourcesPlugin.getWorkspace();
		java.net.URI linksDslUri = workspace.getRoot().findMember("/"+linksDslFile).getLocationURI();
		...
		while (((sCurrentLine=br.readLine()) != null)&& sCurrentLine.startsWith("//alias"))
		{
			...
			for(int i=0;i<inputModelsResourcesList.size()&&!foundCorrectResource;i++)
			{
				...
				EClassifier rootClassifier=(EClassifier)r.getContents().get(0).eClass();
		...
		Iterator<Map.Entry<String,Resource>> iter = inputModelsAliasMapToResource.entrySet().iterator();
		while (iter.hasNext())
		{
			Entry<String, Resource> tempEntry=iter.next();
			Resource modelResource=tempEntry.getValue();
			EmfModel inputModel = createEmfModelByURI(tempEntry.getKey(), modelResource.getURI().toString(), inputmodelsAliasMapMetamodelUri.get(tempEntry.getKey()), true, false);
